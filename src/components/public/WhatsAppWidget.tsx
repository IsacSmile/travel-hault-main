...
export default function WhatsAppWidget() {
  ...
  return (
    <div className="fixed bottom-6 right-6 z-50 flex flex-col items-end font-sans">
      ...
      {open && activeChannel && (
        <div className="mb-3 w-80 sm:w-84 bg-white rounded-3xl shadow-2xl border border-gray-200 overflow-hidden animate-in zoom-in-95 duration-200">
          {/* Header */}
          <div
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <span className="text-[10px] text-emerald-300 font-bold block flex items-center gap-1">
                  <span className="w-2 h-2 rounded-full bg-emerald-400 animate-ping" /> Online & Responsive
                </span>
              </div>
            </div>
          ...
          <form
            ...
            className="p-4 bg-gray-50 text-xs text-gray-700 space-y-3"
          >
            <div className="p-3 bg-white rounded-2xl border border-gray-200 shadow-xs leading-relaxed text-gray-600">
              {activeChannel === 'whatsapp'
            ...
            <div className="relative">
              <textarea
                ...
                className="w-full p-3 bg-white border border-gray-300 focus:border-[#c9a15a] rounded-2xl text-xs outline-none resize-none shadow-inner"
              />
            </div>

            <button
              type="submit"
              className={`w-full py-3 min-h-[44px] text-white font-extrabold text-xs rounded-full shadow-md transition-all duration-150 flex items-center justify-center gap-2 hover:-translate-y-0.5 active:translate-y-0 ${
                activeChannel === 'whatsapp'
      ...
      {/* Stacked Circular Action Buttons (when expanded) */}
      {open && (
        <div className="flex flex-col items-center gap-3 mb-3 animate-in fade-in slide-in-from-bottom-3 duration-200">
          {/* Messenger Icon Button */}
          {settings.messengerEnabled && (
            <button
              onClick={() => setActiveChannel(activeChannel === 'messenger' ? null : 'messenger')}
              className={`w-12 h-12 rounded-full bg-[#0084FF] hover:bg-[#006FDF] text-white flex items-center justify-center shadow-xl transition-all duration-200 hover:scale-110 relative group min-h-[44px] ${
                activeChannel === 'messenger' ? 'ring-4 ring-blue-200 scale-110' : ''
              }`}
              title="Chat on Messenger"
            >
              <MessengerIcon className="w-6 h-6" />
              <span className="absolute right-14 top-1/2 -translate-y-1/2 px-3 py-1 bg-gray-900 text-white text-xs font-bold rounded-lg shadow-lg whitespace-nowrap opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none">
                Messenger Chat
              </span>
            </button>
          )}
          ...
          {settings.whatsappEnabled && (
            <button
              ...
              className={`w-12 h-12 rounded-full bg-emerald-500 hover:bg-emerald-600 text-white flex items-center justify-center shadow-xl transition-all duration-200 hover:scale-110 relative group min-h-[44px] ${
                activeChannel === 'whatsapp' ? 'ring-4 ring-emerald-200 scale-110' : ''
              }`}
      ...
      {/* Main Trigger Launcher Button (Morphs to X when open) */}
      <button
        onClick={() => {
          if (open) {
            setOpen(false);
        ...
        className={`w-14 h-14 rounded-full flex items-center justify-center shadow-2xl transition-all duration-300 hover:scale-105 active:scale-95 min-h-[44px] ${
          open
            ? 'bg-[#1a1815] text-white hover:bg-black'
            : 'bg-emerald-500 hover:bg-emerald-600 text-white'
        }`}
        title={open ? 'Close Chat Menu' : 'Open Support Chat'}
      >
        {open ? (
          <X className="w-7 h-7" />
        ) : (
