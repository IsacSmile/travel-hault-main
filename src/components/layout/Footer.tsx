    ...
    async function loadSettings() {
      try {
        ...
        if (res.ok) {
          ...
          if (typeof data.whatsappEnabled !== 'undefined') {
            setWhatsappEnabled(Boolean(data.whatsappEnabled));
          }
          if (data.messengerLink) setMessengerLink(data.messengerLink);
          if (typeof data.messengerEnabled !== 'undefined') {
            setMessengerEnabled(Boolean(data.messengerEnabled));
          }
  ...
  return (
    <footer className="bg-white text-[#1a1815] border-t border-gray-200 font-sans pt-12 pb-8 relative">
      ...
      <div className="fixed bottom-6 right-6 z-50 flex flex-col-reverse items-center gap-3">
        ...
        {whatsappEnabled && (
          <a
            href={`https://wa.me/${whatsappNumber.replace(/[^0-9]/g, '') || '917407524498'}?text=Hi%20Travel%20%26%20Hault!%20I%20want%20to%20plan%20a%20trip.`}
            target="_blank"
            rel="noopener noreferrer"
        ...
        {messengerEnabled && (
          <a
            href={`https://m.me/${messengerLink || 'travelhault'}`}
            target="_blank"
            rel="noopener noreferrer"
